...
const defaultRenderFunctions = {
  ...
  p: (node, children, parents, styles) =>
    <Text key={AstRenderer.getUniqueID()} style={[markdownStyles.paragraph, styles.paragraph]}>
      {children}
    </Text>,
  ...
  li: (node, children, parents, styles) => {
    if (AstRenderer.hasParents(parents, 'ul')) {
      return (
        <Text key={AstRenderer.getUniqueID()} style={[markdownStyles.listUnorderedItem, styles.listUnorderedItem]}>
