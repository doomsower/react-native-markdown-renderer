...
const defaultRenderFunctions = {
  ...
  li: (node, children, parents, styles) => {
    // If list item contains only 1 paragraph, render content of this paragraph to avoid View nested in Text error
    let liChildren = children;
    if (node.children && node.children.length === 1 && node.children[0] && node.children[0].type === 'p') {
      liChildren = children[0].props.children;
    }
    if (AstRenderer.hasParents(parents, 'ul')) {
      return (
        <View key={AstRenderer.getUniqueID()} style={[markdownStyles.listUnorderedItem, styles.listUnorderedItem]}>
          ...
          <Text style={[markdownStyles.listItem, styles.listItem]}>
            {liChildren}
          </Text>
        </View>
      );
    ...
    if (AstRenderer.hasParents(parents, 'ol')) {
      return (
        <Text key={AstRenderer.getUniqueID()} style={[markdownStyles.listOrderedItem, styles.listOrderedItem]}>
    ...
    return (
      <Text key={AstRenderer.getUniqueID()} style={[markdownStyles.listItem, styles.listItem]}>
        {children}
      </Text>
    );
  },
